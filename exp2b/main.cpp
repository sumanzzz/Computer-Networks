...
int main()
{
    ...
    for(int i=n;i<n+m-1;i++) data.push_back(cdata[i]);
    ...
    if(ch == 'Y' || ch =='y'){
        int p;
        cout<<"Enter the position to change bit (1-indexted):";
        cin>>p;
        if(p-1 < 0 || p-1>n+m-2) cout<<"Invalid Position!!"<<endl;
        else data[p-1] ^= 1;
    }
    cout<<"The message recevied is:";
    for(int bit : data) cout<<bit<<" ";
    cout<<endl;
    ...
    if(flag) cout<<"No Error ---> Successfully  Transfered"<<endl;
